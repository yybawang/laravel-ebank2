...
trait TransferConcerns
{
    ...
    public function transfer(int $user_id, float $amount, string $reason_code, string $description = '', ?array $business_data = []): int {
        ...
        $transfer_id = Cache::lock('ebank@transfer:'.$wallet->id)->block(10, fn() => DB::transaction(function() use ($user_id, $amount, $reason, $description, $business_data){
            $wallet = $this->getWallet($user_id, $reason->currency_id, $reason->identity_id);
    ...
    public function faceToFace(int $from_user_id, int $to_user_id, float $amount, string $from_reason_code, string $to_reason_code, string $description = '', ?array $business_data = []): void {
        ...
        Cache::lock('ebank@faceToFace:'.$from_wallet->id.':'.$to_wallet->id)->block(10, fn() => DB::transaction(function() use ($from_user_id, $to_user_id, $amount, $from_reason, $to_reason, $description, $business_data){
            $from_wallet = $this->getWallet($from_user_id, $from_reason->currency_id, $from_reason->identity_id);
            $to_wallet = $this->getWallet($to_user_id, $to_reason->currency_id, $to_reason->identity_id);
