...
//            abort_if($amount < 0 && $wallet->balance < -$amount, 422, '钱包 '.$wallet->id.' 可用余额不足');
    ...
    public function faceToFace(int $from_user_id, int $to_user_id, float $amount, string $from_reason_code, string $to_reason_code, string $description = '', ?array $business_data = null): void {
        ...
        Cache::lock('ebank@faceToFace:'.'ebank@transfer:'.$from_user_id.':'.$from_reason->currency_id.':'.$from_reason->identity_id.':'.$to_user_id.':'.$to_reason->currency_id.':'.$to_reason->identity_id)->block(10, fn() => DB::transaction(function() use ($from_user_id, $to_user_id, $amount, $from_reason, $to_reason, $description, $business_data){
